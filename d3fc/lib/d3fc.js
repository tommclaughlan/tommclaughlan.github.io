...
var shaderBuilder = (function (base) {
    ...
    build.insertHeader = function (header, before) {
        var bi = shaderHeaders.indexOf(before);
        shaderHeaders.splice(bi >= 0 ? bi : arr.length, 0, header);
        return build;
    };

    build.appendBody = function (header) {
        shaderBodies.push(header);
        return build;
    };

    build.insertBody = function (header, before) {
        var bi = shaderBodies.indexOf(before);
        shaderBodies.splice(bi >= 0 ? bi : arr.length, 0, header);
        return build;
    };
...
var buffer = (function (gl, program) {
    var buf = gl.createBuffer();

    var buffer = function buffer(data, name, components) {
...
var programBuilder = (function () {
    ...
    var bindAttributes = function bindAttributes(gl, program, attributes) {
        Object.keys(attributes).forEach(function (key) {
            var a = attributes[key];

            var buf = buffer(gl, program);
            buf(a, key, a.length / numElements);
        });
    };

    var bindUniforms = function bindUniforms(gl, program, uniforms) {
     ...
     */
    var build = function build(gl) {
        if (newProgram(gl, program, vertexShader(), fragmentShader())) {
            program = createProgram(gl, vertexShader(), fragmentShader());
        }
        gl.useProgram(program);

        bindAttributes(gl, program, attributes);
        bindUniforms(gl, program, uniforms);

        return program;
    };

    build.uniform = function (key, value) {
        uniforms[key] = value;
        return build;
    };

    build.attribute = function (key, value) {
    ...
    build.numElements = function () {
        if (!arguments.length) {
            return numElements;
        }
        numElements = arguments.length <= 0 ? undefined : arguments[0];
        return build;
    };

    build.bindAttributes = function () {
        if (!arguments.length) {
            return bindAttributes;
        }
        bindAttributes = arguments.length <= 0 ? undefined : arguments[0];
        return build;
    };

    build.bindUniforms = function () {
        if (!arguments.length) {
            return bindUniforms;
        }
        bindUniforms = arguments.length <= 0 ? undefined : arguments[0];
        return build;
    };
    ...
    function newProgram(gl, program, vertexShader, fragmentShader) {
        ...
        var vSource = gl.getShaderSource(shaders[0]);
        var fSource = gl.getShaderSource(shaders[1]);

        return vertexShader !== vSource || fragmentShader !== fSource;
    }

    function createProgram(gl, vertexShader, fragmentShader) {
        var vShader = loadShader(gl, vertexShader, gl.VERTEX_SHADER);
        var fShader = loadShader(gl, fragmentShader, gl.FRAGMENT_SHADER);
...
var webGlBase = (function (gl, programBuilder) {

    programBuilder(gl);

    var draw = function draw() {
        var mode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : gl.TRIANGLES;

        resizeCanvas();
        gl.enable(gl.BLEND);
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
        gl.drawArrays(mode, 0, programBuilder.numElements());
    };
...
var clipSpace = {
    ...
    body: "gl_Position.xy = (2.0 * (gl_Position.xy / uResolution)) - 1.0;\n    gl_Position.y = -1.0 * gl_Position.y;"
};
...
var vertexShaderSnippets = Object.freeze({
	clipSpace: clipSpace,
	scale: scale,
	translate: translate,
...
var circle = {
    header: "varying float vSize;",
    body: "float distance = length(2.0 * gl_PointCoord - 1.0) * vSize;\n    if (distance > vSize) {\n        discard;\n        return;\n    }"
};
...
var fragmentShaderSnippets = Object.freeze({
	circle: circle,
...
var circlePointShader = (function () {
    ...
    vertexShader.appendHeader(point$1.header).appendHeader(clipSpace.header).appendBody(point$1.body).appendBody(clipSpace.body);
...
var point = (function () {
    ...
    var point = function point(data) {
        ...
        var shaderBuilder$$1 = circlePointShader();
        program.vertexShader(shaderBuilder$$1.vertex());
        program.fragmentShader(shaderBuilder$$1.fragment());
        program.numElements(filteredData.length);

        var points = new Float32Array(filteredData.length * 3);
        var pi = 0;
        filteredData.forEach(function (d, i) {
            var values = base.values(d, i);
        ...
        program.attribute('aVertexPosition', points);
        program.uniform('uResolution', [context.canvas.width, context.canvas.height]);

        base.decorate()(program, filteredData, 0);

        var draw = webGlBase(context, program);
        draw(context.POINTS);
    };
...
var cartesianChart = (function () {
    ...
    var cartesian = function cartesian(selection$$1) {
        ...
        selection$$1.each(function (data, index, group) {
            ...
            webglDataJoin(container, webglPlotArea ? [data] : []).classed('plot-area', true).on('draw', function (d, i, nodes) {
                var canvas = d3Selection.select(nodes[i]).select('canvas').node();
